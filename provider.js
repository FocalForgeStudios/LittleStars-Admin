...
let providerDashboardStarted = false;

function startProviderDashboard() {
  if (providerDashboardStarted) return;
  providerDashboardStarted = true;
  initDashboard();
}

document.addEventListener('lse:providerReady', startProviderDashboard);
if (window.__lseProviderReady || document.getElementById('dashboardRoot')?.style.display === 'block') {
  startProviderDashboard();
}
...
function initDashboard() {
  ...
  async function refreshAll() {
    try {
      ...
      alert('Dashboard data could not load. Please refresh the page or check your provider account permissions.');
    }
  }
